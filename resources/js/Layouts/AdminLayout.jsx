    ...
    return (
        <div className="flex h-screen bg-gray-100 font-sans relative overflow-hidden">
            ...
            <aside className={`
                ...
                bg-[#2563eb] text-white transition-all duration-300 flex flex-col shadow-2xl md:shadow-xl
            `}>
                
                <div className="p-4 border-b border-blue-500/30 flex items-center justify-between h-20 shrink-0">
                    {showText && (
                        <div>
                    ...
                    <button onClick={() => setIsMobileOpen(false)} className="md:hidden p-2 text-blue-200 hover:text-white rounded-lg hover:bg-blue-600">
                ...
                <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-1.5 scrollbar-thin scrollbar-thumb-blue-400 scrollbar-track-transparent">
                    
                    <Link href="/admin/dashboard" className={`flex items-center p-2.5 rounded-lg transition-all ${!showText && 'justify-center'} ${isActive('/admin/dashboard') ? 'bg-white text-blue-600 shadow-sm font-semibold' : 'text-blue-100 hover:bg-blue-700 font-medium'}`}>
                    ...
                    <div className="text-[10px] font-semibold text-blue-300 mt-5 mb-1.5 px-2.5 uppercase tracking-wider truncate">{showText ? 'Keuangan' : '...'}</div>
                    
                    <Link href="/admin/financial-reports" className={`flex items-center p-2.5 rounded-lg transition-all ${!showText && 'justify-center'} ${isActive('/admin/financial-reports') ? 'bg-white text-blue-600 shadow-sm font-semibold' : 'text-blue-100 hover:bg-blue-700 font-medium'}`}>
                        <svg className="w-[18px] h-[18px] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
                        {showText && <span className="ml-2.5 text-xs truncate">Laporan Keuangan</span>}
                    </Link>
                    
                    <Link href="/admin/expenses" className={`flex items-center p-2.5 rounded-lg transition-all ${!showText && 'justify-center'} ${isActive('/admin/expenses') ? 'bg-white text-blue-600 shadow-sm font-semibold' : 'text-blue-100 hover:bg-blue-700 font-medium'}`}>
                        <svg className="w-[18px] h-[18px] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6" /></svg>
                        {showText && <span className="ml-2.5 text-xs truncate">Pengeluaran</span>}
                    </Link>

                    <div className="text-[10px] font-semibold text-blue-300 mt-5 mb-1.5 px-2.5 uppercase tracking-wider truncate">{showText ? 'Lainnya' : '...'}</div>
                    
                    <Link href="/admin/services" className={`flex items-center p-2.5 rounded-lg transition-all ${!showText && 'justify-center'} ${isActive('/admin/services') ? 'bg-white text-blue-600 shadow-sm font-semibold' : 'text-blue-100 hover:bg-blue-700 font-medium'}`}>
                        ...
                        {showText && <span className="ml-2.5 text-xs truncate">Kelola Layanan</span>}
                    </Link>

                    <Link href="/admin/cashiers" className={`flex items-center p-2.5 rounded-lg transition-all ${!showText && 'justify-center'} ${isActive('/admin/cashiers') ? 'bg-white text-blue-600 shadow-sm font-semibold' : 'text-blue-100 hover:bg-blue-700 font-medium'}`}>
                        ...
                        {showText && <span className="ml-2.5 text-xs truncate">Kelola Pengguna</span>}
                    </Link>
                    
                    <Link href="/logout" method="post" as="button" className={`w-full flex items-center p-2.5 rounded-lg transition-all ${!showText && 'justify-center'} text-blue-100 hover:bg-blue-700 font-medium mt-4`}>
                ...
                <div className="p-3 border-t border-blue-400/20 bg-blue shrink-0">
                    <div className={`flex items-center ${!showText ? 'justify-center' : ''}`}>
                        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-500 border border-blue-400 flex items-center justify-center shadow-sm">
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden w-full relative">
                <header className="bg-[#3b82f6] text-white h-20 px-4 md:px-8 flex items-center justify-between shadow-md z-10 shrink-0">
                    <div className="flex items-center">
                        <button 
                            onClick={() => setIsMobileOpen(true)}
                            className="mr-3 md:hidden p-2 rounded-lg bg-blue-600 hover:bg-blue-700 focus:outline-none transition-colors"
                        >
                    ...
                    <div className="text-xs md:text-sm font-medium text-blue-100 hidden sm:block">{today}</div>
                </header>
