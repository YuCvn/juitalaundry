...
export default function Orders() {
    ...
    const activeOrders = orders.filter(o => o.status_order !== 'sudah diambil');

    const countSemua = activeOrders.length;
    const countMenunggu = activeOrders.filter(o => o.status_order === 'menunggu').length;
    const countProses = activeOrders.filter(o => o.status_order === 'dalam proses').length;
    const countSelesai = activeOrders.filter(o => o.status_order === 'selesai').length;

    const filteredOrders = activeOrders.filter(order => {
        if (activeFilter === 'semua') return true;
        if (activeFilter === 'menunggu') return order.status_order === 'menunggu';
        if (activeFilter === 'proses') return order.status_order === 'dalam proses';
        if (activeFilter === 'selesai') return order.status_order === 'selesai';
        return true;
    });

    const getStatusStyle = (status) => {
        switch (status?.toLowerCase()) {
            case 'menunggu': return 'bg-amber-50 text-amber-600 border-amber-200 focus:ring-amber-500';
            case 'dalam proses': return 'bg-blue-50 text-blue-600 border-blue-200 focus:ring-blue-500';
            case 'selesai': return 'bg-emerald-50 text-emerald-600 border-emerald-200 focus:ring-emerald-500';
            case 'sudah diambil': return 'bg-gray-50 text-gray-600 border-gray-200 focus:ring-gray-500';
